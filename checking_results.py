...
# Visualize Motifs in a Word Cloud
motif_string = " ".join(motifs)
wordcloud = WordCloud(width=800, height=400, background_color="white").generate(motif_string)

plt.figure(figsize=(10, 6))
plt.imshow(wordcloud, interpolation='bilinear')
plt.axis('off')
plt.title("Top 10 Motifs")
plt.show()
...
sns.heatmap(conf_matrix, annot=True, fmt='d', cmap='Blues', xticklabels=["No Error", "Error"], yticklabels=["No Error", "Error"])
plt.title("Confusion Matrix")
plt.xlabel("Predicted Label")
plt.ylabel("True Label")
plt.show()

# Step 11: Display important features
...
# Step 12: Save everything into a single pickle file
to_save = {
    "model": model,
