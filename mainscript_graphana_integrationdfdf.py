...
# Load the ML model
try:
    # Load the model and its associated components
    with open('model_and_features.pkl', 'rb') as f:
        model_and_features = pickle.load(f)
    
    # Extract the model and vectorizers from the dictionary
    model = model_and_features.get('model')
    motif_vectorizer = model_and_features.get('motif_vectorizer')
    tfidf_vectorizer = model_and_features.get('tfidf_vectorizer')

    # Validate that the model is properly loaded
    if model is not None and hasattr(model, 'predict'):
        print("Model loaded successfully with 'predict' method.")
    else:
        raise ValueError("The loaded model is invalid or does not have a 'predict' method.")
    
    # Ensure vectorizers are also properly loaded
    if not motif_vectorizer or not tfidf_vectorizer:
        raise ValueError("One or both vectorizers are missing from the loaded file.")
    
except Exception as e:
    print(f"Error loading model and features: {e}")
    model = None
    motif_vectorizer = None
    tfidf_vectorizer = None


def process_log_for_anomaly(log_data):
    try:
        if not model or not motif_vectorizer or not tfidf_vectorizer:
            raise ValueError("Model or vectorizers are not loaded properly.")

        # Convert the log data to a DataFrame
        df = pd.DataFrame([log_data])  # Single-row DataFrame
        required_features = ['timestamp', 'ip', 'api_endpoint', 'log_level', 'user_agent', 'response_code']

        # Vectorize the log message
        message = log_data.get('message', '')
        motif_features = motif_vectorizer.transform([message])
        tfidf_features = tfidf_vectorizer.transform([message])
